...
class ProductionProcess:
    ...
    def _run(self):
        while True:
            if self.is_running:
                # Als de tank onder de 3 ton zakt, gecontroleerd op 'Suspended' gaan.
...
class BatchProductionProcess:
    ...
    def _run(self):
        while True:
            if self.is_running:
                # Als de voorraad onder de vereiste batch-capaciteit zit, tijdelijk 'Suspended'
...
class CampaignController:
    ...
    def _monitor_loop(self):
        while True:
            # CORRECTIE: Strikte hysteresis campagne-modus. 
            # Start pas als de tank écht bijna vol is (>= 90%).
            trigger_high = self.trigger_node.container.capacity * 0.90
            # Stop de campagne zodra er niet eens meer genoeg in zit voor 1 batch (capaciteit hydrogenation = 16.5 ton)
            trigger_low = 17.0 
            
            # Controleer de huidige status van de processen (we pakken de eerste uit de lijst)
            currently_running = self.processes[0].is_running
            
            if not currently_running and self.trigger_node.level >= trigger_high:
                # Tank is vol genoeg -> Start de campagne!
                for proc in self.processes:
                    proc.is_running = True
            
            elif currently_running and self.trigger_node.level <= trigger_low:
                # Tank is leeg gedraaid -> Stop de campagne hard, wissel naar Suspended/Idle
                # en wacht tot de tank via vrachtwagens weer helemaal vult tot 90%.
                for proc in self.processes:
...
class EmmerichDistillationController:
    ...
    def _monitor_loop(self):
        while True:
            if self.trigger_node.free_space <= 15:
                for proc in self.processes:
                    proc.is_running = True
...
def run_visual_simulation():
    ...
    network_nodes = {
        ...
        'Tank4_Hydro_Buffer':StorageTank(env, 'Hydro Buffer', capacity_m3=292.4, density=0.9, init_fill_pct=0.0), 
        ...
        'Tank6_Purified':    StorageTank(env, 'Final Purified', capacity_m3=2000, density=0.9, init_fill_pct=0.0)   
    }
    ...
    # Gecorrigeerde controller gekoppeld aan de hydrogenation unit
    CampaignController(env, trigger_node=network_nodes['Tank3_Monomer_Ert'], controlled_processes=[proc_hydrogenation])
