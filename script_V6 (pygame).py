...
class TransportLink:
    def __init__(self, env, name, source_node, dest_node, batch_size, transit_hours, trigger_pct=0.0):
        ...
        self.trigger_pct = trigger_pct  # Nieuw: Percentage waarbij de transportstroom pas opstart
    ...
    def _run(self):
        while True:
            ...
            if self.shipping_allowed and self.source.level >= self.batch_size:
                ...
                # Geef de vrachtwagens een kleine staffeling (bijv. elk half uur één) in plaats van allemaal tegelijk
                yield self.env.timeout(0.5)
            else:
                yield self.env.timeout(1.0)
...
class CampaignController:
    ...
    def _monitor_loop(self):
        while True:
            trigger_threshold = self.trigger_node.container.capacity * 0.90
...
class DownstreamDistillationController:
    ...
    def _monitor_loop(self):
        while True:
            # Start the slow final column as soon as Tank 7 gets its first major drop
            if self.trigger_node.level >= 20.0:
                for proc in self.processes:
                    proc.is_running = True
            
            # Shut down gracefully when empty to await the next campaign shipment
            if self.trigger_node.level <= 1.0:
                for proc in self.processes:
                    proc.is_running = False
...
# ==============================================================================
# LAYER 6: PYGAME VISUALIZATION INTERFACE (SCENARIO 2: RETURN TO EMMERICH)
# ==============================================================================
...
def run_visual_simulation():
    ...
    network_nodes = {
        ...
        'Tank5_Ert_Buffer': StorageTank(env, 'Ertvelde Buffer', capacity_m3=290, density=0.9, init_fill_pct=0.0), 
        'Tank7_Em_Return':  StorageTank(env, 'EM Return Feed', capacity_m3=82, density=0.9, init_fill_pct=0.0), 
        'Tank6_Purified':   StorageTank(env, 'Final Purified', capacity_m3=280, density=0.9, init_fill_pct=0.0)   
    }
    ...
    # Outbound Logistics (Top-level clean horizontal bridge)
    ...
    proc_ert_camp = ProductionProcess(env, 'Ert_Campaign', network_nodes['Tank3_Monomer_Ert'], network_nodes['Tank5_Ert_Buffer'], flow_rate=2.5, process_yield=(0.99 * 0.743), active_by_default=False)
    
    # Inbound Logistics (Bottom-level clean horizontal return)
    logistics_inbound = TransportLink(
        env, 
        'Inbound_Return_Link', 
        network_nodes['Tank5_Ert_Buffer'], 
        network_nodes['Tank7_Em_Return'], 
        batch_size=24,          # Vrachtwagengrootte blijft 24 ton
        transit_hours=3, 
        trigger_pct=0.90        # <--- Dit lijnt de vrachtwagens perfect uit met het vatontwerp!
    )
    ...
    DownstreamDistillationController(env, trigger_node=network_nodes['Tank7_Em_Return'], controlled_processes=[proc_em_frac])
    EmmerichDistillationController(env, trigger_node=network_nodes['Tank1_CrudeIso'], controlled_processes=[proc_em_dist])
    
    all_production_lines = [proc_reactor, proc_em_dist, proc_ert_camp, proc_em_frac]
    ...
    pygame.display.set_caption("Supply Chain Optimization - Clean U-Shape Flow")
    ...
    #SYMMETRIC GRID
    tank_positions = {
        # ROW 1: FORWARD CHAIN (Left to Right)
        ...
        # ROW 2: RETURN CHAIN (Right to Left)
        'Tank5_Ert_Buffer':  (720, 440, 85, 120),
    ...
    while running and env.now < sim_duration:
        ...
        # Crisp clean dashboard background
        screen.fill((244, 246, 249))

        # 1. PHYSICAL GEOGRAPHICAL BOUNDARIES
        pygame.draw.rect(screen, (235, 242, 250), (25, 95, 500, 515), border_radius=12) # Germany Hub
        pygame.draw.rect(screen, (238, 252, 242), (660, 95, 510, 515), border_radius=12) # Belgium Process Plant
        
        screen.blit(font_title.render("SITE 1: EMMERICH (Germany)", True, (44, 62, 80)), (40, 110))
        screen.blit(font_title.render("SITE 2: ERTVELDE (Belgium)", True, (44, 62, 80)), (675, 110))

        # 2. INTRA-SITE PIPELINES (Internal Process Flows)
        processes_draw_data = [
            (proc_reactor, 135, 220, True),  # Tank 0 -> 1
            (proc_em_dist, 295, 220, True),  # Tank 1 -> 2
            (proc_ert_camp, 762, 320, False), # Tank 3 -> 5 (Vertical Drop Downwards)
            (proc_em_frac, 295, 500, False)   # Tank 7 -> 6 (Leftwards)
        ]

        for proc, px, py, horizontal in processes_draw_data:
            color = status_colors.get(proc.status, (127, 143, 166))
            lbl_status = font_bold.render(proc.status, True, color)
            
            if proc.name == 'Ert_Campaign':
                # Vertical process line drawing
                pygame.draw.line(screen, color, (px, py), (px, py + 80), 5)
                pygame.draw.polygon(screen, color, [(px - 6, py + 72), (px, py + 82), (px + 6, py + 72)])
                screen.blit(font.render(proc.name, True, (110, 120, 135)), (px + 15, py + 25))
                screen.blit(lbl_status, (px + 15, py + 42))
            else:
                # Horizontal process line drawing
                pygame.draw.line(screen, color, (px, py), (px + 75, py), 5)
                if horizontal:
                    pygame.draw.polygon(screen, color, [(px + 67, py - 6), (px + 77, py), (px + 67, py + 6)])
                else:
                    pygame.draw.polygon(screen, color, [(px + 8, py - 6), (px, py), (px + 8, py + 6)])
                screen.blit(font.render(proc.name, True, (110, 120, 135)), (px + 10, py - 28))
                screen.blit(lbl_status, (px + 12, py + 10))

        # LOGISTICS LINK 1: OUTBOUND CROSS-BORDER HIGHWAY (Horizontal Top)
        ...
        # LOGISTICS LINK 2: RETURN CROSS-BORDER HIGHWAY (Horizontal Bottom)
        pygame.draw.line(screen, (170, 178, 185), (720, 500), (455, 500), 3)
        if logistics_inbound.active_trucks_in_transit > 0:
            pygame.draw.rect(screen, (241, 196, 15), (560, 488, 45, 24), border_radius=4)
            screen.blit(font_bold.render(f"x{logistics_inbound.active_trucks_in_transit}", True, (0, 0, 0)), (573, 492))
        screen.blit(font.render("Return Freight (3h)", True, (52, 73, 94)), (545, 515))

        # 3. DRAW STORAGE ASSETS (Rendered over the pipeline connections)
        ...
        screen.blit(font_title.render("SCENARIO B: U-SHAPE OPTIMIZED SUPPLY CHAIN CONTROL CENTER", True, (255, 255, 255)), (25, 18))
        ...
        screen.blit(font_title.render(f"Centralized Pure Inventory Stockpile: {final_stock:.2f} Tons", True, (39, 174, 96)), (740, 642))

        pygame.display.flip()

    pygame.quit()
    telemetry.export_to_csv('enhanced_supply_chain_logbook.csv')
    print("\n[Visual Patch] Clean U-Shape layout simulation finished.")
