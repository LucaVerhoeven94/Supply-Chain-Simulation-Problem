...
class EmmerichDistillationController:
    ...
    def _monitor_loop(self):
        while True:
            # Trigger de distillatie als Tank 1 bijna vol zit (minder dan 10 ton ruimte over)
            if self.trigger_node.free_space <= 10.0:
                for proc in self.processes:
                    proc.is_running = True
            
            # Zet de distillatie stil als Tank 1 helemaal leeg is getrokken
            if self.trigger_node.level <= 1.0:
                for proc in self.processes:
                    proc.is_running = False
...
if __name__ == "__main__":
    ...
    proc_em_dist = ProductionProcess(
        ...
        dest_node=network_nodes['Tank2_Monomer_Em'], flow_rate=2.5, process_yield=0.86, active_by_default=False
    )
