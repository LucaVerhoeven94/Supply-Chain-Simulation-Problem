...
class ProductionProcess:
    def __init__(self, env, name, source_node, dest_node, flow_rate, process_yield, active_by_default=True, max_production=None, empty_status="Starved"):
        self.env = env
        self.name = name
    ...
    def _run(self):
        while True:
            # Check if the maximum production limit has been reached
...
class TransportLink:
    def __init__(self, env, name, source_node, dest_node, batch_size, transit_hours, trigger_pct=0.0, upstream_process=None):
        self.env = env
        self.name = name
        self.source = source_node
        self.dest = dest_node
    ...
    def _run(self):
        while True:
            ...
            if self.trigger_pct > 0.0:
                ...
                # Open gate if threshold reached OR if campaign is ending (flush)
                if not self.shipping_allowed and (self.source.level >= threshold or is_campaign_end):
                    self.shipping_allowed = True
                    
                # Only close the gate if campaign is STILL RUNNING
...
class CampaignController:
    def __init__(self, env, trigger_node, controlled_processes, trigger_high_pct, trigger_low_tons, upstream_process=None):
        self.env = env
    ...
    def _monitor_loop(self):
        while True:
            ...
            # End-of-Campaign: Force ON and retire the controller
            if is_campaign_end:
                for proc in self.processes:
                    proc.is_running = True
                while True:
                    yield self.env.timeout(10.0) # Slaap voor altijd, proces regelt het zelf verder
            
            # Start conditie: Hysteresis bereikt
            if not currently_running and self.trigger_node.level >= high_threshold:
                for proc in self.processes:
                    proc.is_running = True
            
            # Stop conditie: Ondergrens bereikt
...
class EmmerichDistillationController:
    def __init__(self, env, trigger_node, controlled_processes, free_space_trigger_tons, low_trigger_tons, upstream_process=None):
        self.env = env
        self.trigger_node = trigger_node
        self.processes = controlled_processes
...
class QuarterlyCampaignController:
    def __init__(self, env, accumulation_tank, controlled_processes, threshold_tons, quarter_hours=2190.0, upstream_process=None):
        self.env = env
        self.tank = accumulation_tank
        self.processes = controlled_processes
        self.threshold = threshold_tons
        self.quarter_hours = quarter_hours
        self.upstream = upstream_process 
        
        self.env.process(self._quarterly_loop())

    def _quarterly_loop(self):
        while True:
            quarter_start_time = self.env.now
            
            # PHASE 1: ACCUMULATION
            for proc in self.processes:
                proc.is_running = False
                
            # Wacht tot de tank vol is OF het kwartaal voorbij is
            while self.tank.level < self.threshold and (self.env.now - quarter_start_time) < self.quarter_hours:
                if self.upstream and self.upstream.status == "Completed":
                    break # Force start! Campaign is over.
                yield self.env.timeout(1.0)
                
            # PHASE 2: PROCESS THE REMAINDER
            for proc in self.processes:
                proc.is_running = True
                
            # End-of-Campaign: Retire the controller, leave processes running
            if self.upstream and self.upstream.status == "Completed":
                while True:
                    yield self.env.timeout(10.0) 
                    
            # Anders, wacht de rest van het kwartaal netjes af
            remaining_time = self.quarter_hours - (self.env.now - quarter_start_time)
            if remaining_time > 0:
...
def run_visual_simulation():
    ...
    network_nodes = {
        ...
        'Tank2_Monomer_Em':  StorageTank(env, 'Distillate EM', capacity_m3=60, density=0.9, init_fill_pct=0.0),
        'Tank3_Monomer_Ert': StorageTank(env, 'Distillate ERT', capacity_m3=360, density=0.9, init_fill_pct=0.0),
        'Tank4_Hydro_Buffer':StorageTank(env, 'Hydrogenated ERT', capacity_m3=292.4, density=0.9, init_fill_pct=0.0), 
        'Tank5_Ert_Buffer':  StorageTank(env, 'Cold Product ERT', capacity_m3=290, density=0.9, init_fill_pct=0.0), 
        'Tank7_Em_Return':   StorageTank(env, 'Cold Product EMM', capacity_m3=82, density=0.9, init_fill_pct=0.0), 
        'Tank6_Purified':    StorageTank(env, 'Final ISA', capacity_m3=2000, density=0.9, init_fill_pct=0.0)   
    }
    ...
    proc_reactor = ProductionProcess(env, 'Reactor', network_nodes['Tank0_RawProduct'], network_nodes['Tank1_CrudeIso'], flow_rate=0.42, process_yield=0.90, max_production=None)
    ...
    logistics_outbound = TransportLink(env, 'Outbound_Link', network_nodes['Tank2_Monomer_Em'], network_nodes['Tank3_Monomer_Ert'], batch_size=24, transit_hours=3, trigger_pct=0.95, upstream_process=proc_reactor)
    ...
    proc_cold_frac = ProductionProcess(env, 'Cold_Fractionation', network_nodes['Tank4_Hydro_Buffer'], network_nodes['Tank5_Ert_Buffer'], flow_rate=2.5, process_yield=0.743, active_by_default=True, empty_status="Suspended")
    logistics_inbound = TransportLink(env, 'Inbound_Return_Link', network_nodes['Tank5_Ert_Buffer'], network_nodes['Tank7_Em_Return'], batch_size=24, transit_hours=3, trigger_pct=0.90, upstream_process=proc_reactor)
    ...
    # ==============================================================================
    # DYNAMIC INITIALIZATION OF CONTROLLERS
    # ==============================================================================
    CampaignController(
        env, 
        trigger_node=network_nodes['Tank3_Monomer_Ert'], 
        controlled_processes=[proc_hydrogenation],
        trigger_high_pct=0.90, 
        trigger_low_tons=17.0,
        upstream_process=proc_reactor
    )
    
    EmmerichDistillationController(
        env, 
        trigger_node=network_nodes['Tank1_CrudeIso'], 
        controlled_processes=[proc_em_dist],
        free_space_trigger_tons=15.0, 
        low_trigger_tons=1.0,
        upstream_process=proc_reactor
    )
    
    QuarterlyCampaignController(
        env, 
        accumulation_tank=network_nodes['Tank5_Ert_Buffer'], 
        controlled_processes=[proc_em_frac],
        threshold_tons=200.0, 
        quarter_hours=2190.0,
        upstream_process=proc_reactor
    )
    # ==============================================================================
    ...
    pygame.display.set_caption("Supply Chain Simulation - Geometrically Audited Layout")
    ...
    status_colors = {
        ...
        "Completed": (52, 152, 219), "Suspended": (149, 165, 166), "Idle": (149, 165, 166)
    }
    ...
    while running:
        ...
        if simulation_active:
            if env.now < sim_duration:
                ...
                print(f"\n[Simulation Complete] CSV Exported. Screen frozen for review at T={env.now}.")
        ...
        for key, node in network_nodes.items():
            ...
            pct = max(0, min(1, node.level / node.container.capacity)) # Safeguard for visualization
            fill_h = pct * h
        ...
        status_text = f"Time: {env.now} / {sim_duration} Hrs" if simulation_active else "SIMULATION COMPLETE"
        screen.blit(font_title.render(status_text, True, (241, 196, 15)), (850, 18))
    ...
    #================================================================================
    # PYGAME LOOP ENDED, EXPORT LOGBOOK
    #================================================================================
